...
class ZergRushBot:
    ...
    '''
    def mining(self, worker):
        if self.mineralMatch[worker] > -1:
            if self.workers[worker].is_carrying_minerals:
                self.workers[worker].move(self.townhalls[0].x-20)
    '''
    ...
    async def on_step(self, bot, iteration):
        ...
        if zerglings >= 100:
            ...
            if bot.structures(UnitTypeId.LAIR).ready.exists and not bot.structures(UnitTypeId.SPIRE).exists and bot.can_afford(UnitTypeId.SPIRE) and bot.already_pending(UnitTypeId.SPIRE) == 0:
                for d in range(4, 15):
                    pos: Point2 = hatch.position.towards(bot.game_info.map_center, d).offset((3, 0))
                    if await bot.can_place_single(UnitTypeId.SPIRE, pos):
                        drone: Unit = bot.workers.closest_to(pos)
                        drone.build(UnitTypeId.SPIRE, pos)
        ...
        for worker in bot.units(UnitTypeId.DRONE):
            if worker == self.defender:
                alive = True
        #################################################################
        if not alive:
            self.defender = 0

        if not self.attacking and not self.defender and (bot.units(UnitTypeId.DRONE) in bot.enemy_units or bot.units(UnitTypeId.PROBE) in bot.enemy_units or bot.units(UnitTypeId.SCV) in bot.enemy_units) and bot.enemy_units.first.distance_to(hatch) <= 10:
            self.defender = bot.workers.closest_to(bot.enemy_units.first)
            self.defender.attack(bot.enemy_units.first)
        
        if self.defender and (self.defender.distance_to(hatch) > 10 or self.attacking):
            minerals: Units = bot.mineral_field.closer_than(10, hatch)
            if minerals:
                mineral: Unit = minerals.closest_to(self.defender)
                self.defender.gather(mineral, queue=False)
                self.defender = 0

        if self.defender and (bot.units(UnitTypeId.DRONE) not in bot.enemy_units and bot.units(UnitTypeId.PROBE) not in bot.enemy_units and bot.units(UnitTypeId.SCV) not in bot.enemy_units):
            minerals: Units = bot.mineral_field.closer_than(10, hatch)
            if minerals:
                mineral: Unit = minerals.closest_to(self.defender)
                self.defender.gather(mineral, queue=False)
        ...
        if bot.minerals > 500:
            for d in range(4, 15):
                pos: Point2 = hatch.position.towards(bot.game_info.map_center, d)
                if await bot.can_place_single(UnitTypeId.HATCHERY, pos):
                    bot.workers.random.build(UnitTypeId.HATCHERY, pos)
                    break

        # While we have less than 16 drones, make more drones
        if bot.can_afford(UnitTypeId.DRONE) and bot.supply_workers+bot.already_pending(UnitTypeId.DRONE) < 16:
        ...
        if (
            ...
            drone: Unit = bot.workers.random
            target: Unit = bot.vespene_geyser.closest_to(drone)
            drone.build_gas(target)

        # If we have no spawning pool, try to build spawning pool
        elif bot.structures(UnitTypeId.SPAWNINGPOOL).amount + bot.already_pending(UnitTypeId.SPAWNINGPOOL) == 0:
            if bot.can_afford(UnitTypeId.SPAWNINGPOOL):
                for d in range(4, 15):
                    pos: Point2 = hatch.position.towards(bot.game_info.map_center, d)
                    if await bot.can_place_single(UnitTypeId.SPAWNINGPOOL, pos):
                        drone: Unit = bot.workers.closest_to(pos)
